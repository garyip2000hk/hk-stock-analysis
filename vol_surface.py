...
由 `options_chain` 拆出嘅逐個行使價 IV，重建每隻股票嘅波幅曲面：
  ...
  前向係數  forward vol ÷ 近月 IV，< 0.85 = **近月被搶貴**（日曆價差機會）
  偏斜      價外 put IV − 價外 call IV，正數 = 跌方保險貴（正常）

審查報告 D2（已修正）：呢支 docstring 同 CLI 原本寫「前向係數 < 0.85 =
遠月被過度定價」，同 `vol_system.py:112` 嘅解讀完全相反 —— 同一個數字喺
兩個檔案裏面得出相反結論，落單方向會搞錯。
由方程看：var₂·t₂ = var₁·t₁ + var_fwd·(t₂ − t₁)，fwd / front < 1
即係中間段嘅隱含波幅低於近月 IV → **近月 IV 相對偏高**，
應該賣近月、買遠月。所以 `vol_system.py` 係對嘅，呢邊已經改正。
...
def _print_one(s: dict) -> None:
    ...
    if ff is not None:
        print()
        if ff < 0.85:
            print(f"  ▶ 前向係數 {ff} < 0.85：近月 IV 被搶貴 → 賣近月／買遠月（日曆價差）")
        elif ff > 1.15:
            print(f"  ▶ 前向係數 {ff} > 1.15：遠月（中間段）被過度定價 → 賣遠月／買近月（反向日曆）")
        else:
